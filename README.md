...
以盗窃罪判例为例，完整跑通「LLM 结构化抽取因子 → 回归/重要性建模 → 对话式量刑建议 Agent」三段流水线：把数据中的隐性模式提炼成可解释的判决经验模型，Agent 据此按重要性追问缺失信息并给出带逐因子拆解的建议，所有刑期数字均来自模型而非 LLM 杜撰。

**核心概念**：知识发现、因子建模、线性回归、数据驱动决策
